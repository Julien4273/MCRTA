...
class photon:
    ...
    def absornot(self, p_abs : float, p_comp):
        ...
        if (rdn <= p_abs):
            ...
            self.wl = wl_0 * (1 + 1/wl_0 * (1-np.cos(theta))) 
    ...
    def coord(self, geo: str, p_abs : float, p_comp : float, nbr_cell : int, nbr_layer : float, tbn_cell : list, tbn_layer : list):
        ...
        while (inside and not_absorbed):
            ...
            if inside :
                ...
                # Calculate the distance traveled between two interaction
                L = dist(x_i, y_i, z_i, x_f, y_f, z_f)
                ...
                # Check the photon is absorbed, if not continue with a new optical depth
                not_absorbed = self.absornot(p_abs,p_comp)
                print("L = "+str(L))
        ...
        return [pos_photon, pos_scat, tele_map]
...
to_plot=[x.coord("cell", 0.1, 0.4, nc, nl, Cells, Layers) for x in photons]
...
for i in range(len(photons)):
    ...
    ax.plot3D(pos[i][0], pos[i][1], pos[i][2], label = "photon "+str(i+1))
    ax.scatter3D(scat[i][0], scat[i][1], scat[i][2], marker = markers[-1])
...
# Plot map of the telescope
fig_maps,ax_maps = plt.subplots(1,1)

for i in range(len(tele_maps)):
    if len(tele_maps[i][0]) != 0 :
        print("Photon "+str(i+1)+" managed to escape !")
        ax_maps.scatter(tele_maps[i][0], tele_maps[i][1], label = "photon "+str(i+1))
    else :
        print("Photon "+str(i+1)+" didn't manage to escape !")
ax_maps.grid()
